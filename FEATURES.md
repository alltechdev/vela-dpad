...
- ✅ **Place photos** — business photo strip **leads the sheet as a hero** (horizontally scrollable, at the top so it's visible at the peek height / in landscape); **tap a photo to open a full-screen, swipeable gallery** with a counter. (~10–20 photos from the search response's preview group at `[1][105][0][1][0][i][6][0]`; the full categorised set loads only via the token-gated `batchexecute` `hspqX` RPC. The per-user session token (`SNlM0e`) approach is now sanctioned, so the full gallery is a buildable follow-up — still keyless/anonymous, no shared API key.)

...
- ✅ **Multi-stop directions (waypoints)** (2026-07-01) — an **"Add stop"** row between From and To (Google-style) opens the same place picker; the pick becomes an intermediate stop and the route recomputes **straight through it**, with each stop **removable** (×). Routes OSRM through `origin → stops… → destination` (`routeVia`, which filters the spurious per-via arrive/depart into one continuous trip) and overlays Google's live in-traffic ETA ratio; a waypointed trip is a **single route** (no alternates). Stops reverse with **⇄**. Device-verified (Everett→Space Needle: adding Alderwood Mall rerouted 43→48 min via a through-path, remove reverted to the direct route + alternates).

...
- ✅ **Popular / busy times** (Google-style histogram in the place sheet, day chips + "busy right now") — **keyless, 2026-06-19.** The catch that fooled us: the keyless **OkHttp** search is bot-degraded (TLS-fingerprint, like photos/transit) and strips the `[84]` histogram, so we wrongly called it login-gated. A real browser engine isn't degraded — a **warmed hidden WebView's same-origin search returns the full ~240 KB response with `[84]`** for an anonymous session. `WebPopularTimesFetcher` (warms google.com→maps, then same-origin-fetches the app's search URL) + `PopularTimesParser`, wired lazily on place select like photos

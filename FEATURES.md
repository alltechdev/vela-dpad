...
- ✅ **Ambient Google POIs on the map** (2026-06-27) — the basemap dots/labels are OSM
  ...
  debounced 500 ms, skipped for pans under 250 m. On-device-verified (Davis: Applebee's, Subway,
...
- ✅ **Multi-stop directions (waypoints)** (2026-07-01) — an **"Add stop"** row between From and To (Google-style) opens the same place picker; the pick becomes an intermediate stop and the route recomputes **straight through it**, with each stop **removable** (×). Routes OSRM through `origin → stops… → destination` (`routeVia`, which filters the spurious per-via arrive/depart into one continuous trip) and overlays Google's live in-traffic ETA ratio; a waypointed trip is a **single route** (no alternates). Stops reverse with **⇄**. Device-verified (Davis→Space Needle: adding Alderwood Mall rerouted 43→48 min via a through-path, remove reverted to the direct route + alternates). *MVP limits (follow-ups): no per-stop "you've arrived" nav announcement (drives the through-route as one path), an off-route reroute heads to the final destination, and stops aren't reorderable yet.*

...
## Map & rendering
- [x] **Typed coordinates drop a pin** - pasting "47.61, -122.33" (or a geo: string) into the search box flies there and drops the reverse-geocoded pin a long-press makes; strict whole-string match so street addresses still search (unit-tested, device-verified).
...
- [x] **Nearby suggestions rank above far matches while typing** - suggestions bucket by ~metro distance (80 km) with Google's relevance order kept inside each bucket, and Google's #1 match or an exact name match is never demoted (a typed city like "tacoma" stays on top; ported upstream d2901d56 + 17f2cdc0).
...
- [x] **Turn instructions keep the road name** ("Turn right onto 164th St SE"), synthesized from OSRM's type/modifier/name/ref/destinations/exits; highways name by ref, ramps read "Take exit 72B toward..."; offline GraphHopper carries names but not refs yet.

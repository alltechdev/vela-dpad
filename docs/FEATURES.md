...
- [x] **Canonical GTFS transit stops on the map with an offline area cache.** At street zoom the bus-stop icons come from Transitous (the agencies' own GTFS stop positions): a blue bus badge with the official stop name, one icon per station. Tapping one opens its live board directly by stop id - no name matching, no language dependence. Where the layer has coverage the basemap's OSM bus icons hide so a stop can't draw twice on different corners; every area browsed online is saved to a small on-disk cache (24 most recent areas) so stops keep drawing offline, and a failed fetch never blanks stops already drawn.

...
osmium extract -b <W,S,E,N> washington-latest.osm.pbf -o seattle.osm.pbf

# 2. build the CH graph (run from the repo root)
./gradlew :tools:graphbuilder:run --args="seattle.osm.pbf seattle-graph"

# 3. zip the output folder -> ship as a release asset; the app downloads + unzips it to internal storage
(cd seattle-graph && zip -qr ../seattle-graph.zip .)
```

Sizing (measured): a Puget-Sound metro = 96 MB `.pbf` → ~53 MB CH graph (~21 MB zipped). Load on the

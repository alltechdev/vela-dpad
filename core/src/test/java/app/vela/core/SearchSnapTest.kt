...
class SearchSnapTest {
    ...
    @Test fun parsesPeopleAlsoSearchFor() {
        val geo = arr(7, 0 to "[null,null,38.95,-122.21]", 6 to "4.7")     // [0]=coords, [6]=rating
        ...
        assertEquals(38.95, sims[0].location.lat, 1e-9)
     ...
     *  place can be backfilled. Regression for "a nail salon showed 4.4 with no count". */
    @Test fun enrichmentLiftsCountAddressRatingFromFocusedNode() {
        val rating = arr(9, 7 to "4.5", 8 to "305")                 // [4][7]=rating, [4][8]=count
        val geo = arr(4, 2 to "38.95", 3 to "-122.21")             // [9][2]/[9][3]=lat/lng
